# Code to calculate the embeddings of nodes accros layers, meant to be used later 
# to make the 'minecraft water' plots of the embeddings in merged_plot.py
import torch
...
def main(args):
    ...
    # Select target node indices uncomment this
    ...
    # Select target node indices (now select all nodes of the target node type)
    target_node_indices = torch.arange(data[target_node_type].num_nodes)
    
    # Get final layer embeddings
    final_layer_embeddings = x_latents_np[-1][target_node_indices]

    # Create a DataFrame and save to CSV
    df = pd.DataFrame(final_layer_embeddings, index=[f'Node {i}' for i in target_node_indices])
    csv_filename = f"3xruns/3xruns_outputs/embeddings/{args.model_type}_{'norm' if args.embedding_option == 'normalize' else 'notnorm'}_{'trained' if args.train else 'propagated'}_{args.num_hops}_layer_{args.dataset}_all_nodes.csv"
