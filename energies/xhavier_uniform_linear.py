...
class RGCN(nn.Module):
    ...
    def init_weights(self):
        for node_type, linear in self.linear_projections.items():
    ...
    def create_linear_projections(self, x_dict):
        ...
        for node_type, x in x_dict.items():
            if node_type not in self.linear_projections:
                in_dim = x.size(1)
                self.linear_projections[node_type] = nn.Linear(in_dim, self.convs[0].in_channels, bias=True).to(device)
        self.init_weights()
...
def main(args):
    ...
    os.makedirs('results/weight_exp_results', exist_ok=True)

    de_adj, de_log_adj = eval_oversmooth_adj(dirichlet_energy_adj, x_latents, edge_index)
    ...
    filename = f'results/weight_exp_results/oversmooth_adj_{args.model_type}_layers_{args.num_hops}_{args.dataset}.csv'
    df_adj.to_csv(filename, index=False)


    de_old, de_log_old = eval_oversmooth_adj(dirichlet_energy_old, x_latents, edge_index)
    ...
    filename = f'results/weight_exp_results/oversmooth_old_{args.model_type}_layers_{args.num_hops}_{args.dataset}.csv'
    df_old.to_csv(filename, index=False)


    de_roth, de_log_roth = eval_oversmooth_adj(dirichlet_energy_roth, x_latents, edge_index)
    df_roth = pd.DataFrame({
        'de': de_roth,
        'de_log': de_log_roth
    })
    filename = f'results/weight_exp_results/oversmooth_roth_{args.model_type}_layers_{args.num_hops}_{args.dataset}.csv'
    df_roth.to_csv(filename, index=False)
