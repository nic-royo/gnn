    ...
    for i, model in enumerate(model_names):
        for j, dataset in enumerate(datasets):
            if dataset == 'dblp' and model == 'ggrgcn':
                df = pd.read_csv(f'results/two_hop/oversmooth_adj_{model}_layers_64_2hop_{dataset}.csv')
            else:
                df = pd.read_csv(f'results/two_hop/oversmooth_adj_{model}_layers_128_2hop_{dataset}.csv')
...
plot_results(
    ...
    datasets=['imdb', 'dblp'],
    title='Two-hop reordered neighborhood',
    output_filename='oversmooth_adj_2_hop'
)
